      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.2fr_0.8fr_1fr] lg:px-8">
        <div>
          <Link to="/" className="inline-flex items-center gap-3">
            <span className="grid h-11 w-11 place-items-center bg-[#d7b46a] text-lg font-black text-black">
              EB
            </span>
            <span>
              <span className="block text-lg font-black text-white">Elite Boxing Club</span>
              <span className="text-sm text-[#d7b46a]">Chicago private boxing studio</span>
            </span>
          </Link>
        ...
        <div>
          ...
          <ul className="mt-4 grid gap-3 text-sm text-stone-400">
            {navItems.map((item) => (
              <li key={item.to}>
                <Link className="transition hover:text-[#f4d58b]" to={item.to}>
                  {item.label}
                </Link>
              </li>
            ))}
